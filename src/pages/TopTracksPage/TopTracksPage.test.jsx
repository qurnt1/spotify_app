  ...
  items: [
    {
      ...
      album: { name: 'Album X', images: [{ url: 'album-x.jpg' }] },
      popularity: 80,
    ...
    },
    {
      ...
      album: { name: 'Album Y', images: [{ url: 'album-y.jpg' }] },
      popularity: 75,
...
describe('TopTracksPage', () => {
  ...
  beforeEach(() => {
    ...
    jest
      .spyOn(spotifyApi, 'fetchUserTopTracks')
      .mockResolvedValue({ data: tracksData, error: null });
  });
  ...
  test.each(sizes)(
    'renders top %i items (limited to 10) and prefixes titles correctly',
    async (n) => {
      jest
        .spyOn(spotifyApi, 'fetchUserTopTracks')
        .mockResolvedValue({ data: buildTracks(n), error: null });
  ...
  test('renders top tracks page', async () => {
    ...
    const heading = await screen.findByRole('heading', {
      level: 1,
      name: `Your Top ${tracksData.total} Tracks of the Month`,
    });
